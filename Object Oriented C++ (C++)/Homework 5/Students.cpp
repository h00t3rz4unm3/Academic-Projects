...
Students::Students(string f, string l, int c, double g, Date d, Date m)
{
	fname = f;
	lname = l;
	credits = c;
...
{
	if (lname == y.lname && fname == y.fname && (dob.getMonthNum() == y.dob.getMonthNum() && dob.getDay() == y.dob.getDay() && dob.getYear() == y.dob.getYear()))
		return true;
	else
		return false;
}
